...
class CameraManager: ObservableObject, CaptureDataReceiver {
    ...
    var isFilteringDepth: Bool // Make depth Filering on all the time - Jessica Kinnevan
    var fpsPublisher = PassthroughSubject<Double, Never>() // Added for FPS - Jessica Kinnevan
    ...
    // updateFPS was created by Team 7 Jessica Kinnevan, Toral Chauhan, Naifa Alqahtani
    // The fpsPublisher.send(fps) statement sends a value (fps) through the fpsPublisher.
    // In Swift, fpsPublisher is an instance of PassthroughSubject<Double, Never>, which is a
    // Combine publisher that can emit values of type Double and never fail (Never indicates no failure).
    // The FPS value ends up on the user interface as a digital number. It is essentially the
    // CPU side FPS, which will never be faster than 30fps because the liDAR data is limited
    // to 30fps. The GPU runs at 60 fps.
    
    func updateFPS(fps: Double) {
        fpsPublisher.send(fps)
    }
}
